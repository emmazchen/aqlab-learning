# -*- coding: utf-8 -*-
"""sklearn_log_reg.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1QJabS5nMKg1RTTwFUDBZd_KBQKG4yPPW
...
Explored using Pipeline and GridSearchCV: <br>
- Pipeline is helpful for linking multiple steps together (including preprocessing and estimators (estimators have to be transformers except in last step)). <br>
- GridSearchCV allows you to find the optimal combination of parameters across many steps. To find the best combo across many steps, you input a pipeline as the estimator and give it a parameter grid of different values to try for each param. <br>
 Note: it uses cross validation so you don't have to split into train and test beforehand. You can specify number of folds you want the data to be divided into.
"""
...
#set and name pipe's steps
steps = [('standardize', StandardScaler()), ('ridge', Ridge())]
pipe = Pipeline(steps)

#this is how you would usually set nested parameters -> we won't because we use GridSearchCV
# pipe.set_params(ridgeCV__alphas=[1e-3, 1e-2, 1e-1, 1])

pipe

#put this all within GridSearchCV
param_grid = {
    "ridge__alpha": [1e-3, 1e-2, 1e-1, 1, 1e2, 1e3, 1e4],
    #if there were more parameters to optimize, put them here
}

#define grid search cv
search = GridSearchCV(pipe, param_grid, n_jobs=2)

#fit to search for optimal parameters
search.fit(X, y)

#different keys to rank by
sorted(search.cv_results_.keys())

#best combo of parameters and the best score obtained
